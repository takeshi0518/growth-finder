...
カフェ店長として 13年の現場経験から生まれた「伸びしろ発見チェックシート」をデジタル化し、スタッフの成長を可視化・共有することで、店舗全体のレベル向上と、店長・スタッフ間の信頼関係構築を目指します。
...
- **サマリータブでの自動ランク算出** (評価点からの集計・ランク判定ロジック)
...
評価作成時に `evaluation_items` テーブルへINSERTする実装になっています。
...
関連Issue: [#88](https://github.com/takeshi0518/Growth-finder/issues/88)
...
Vercelのプロジェクト「Commerce」 のコードリーディングから学んだ**「データフェッチの 3 パターン統一」**を参考にしています。
...
>
> エンジニアリングは目的ではなく手段です。
> 13年の現場経験で見つけた課題を、自分の手で解決していくプロセスそのものが、私のエンジニアとしての価値だと考えています。
