...
> 店長が入力した QHC 評価をもとに、1on1 面談用のフィードバックを自動生成する AI エージェント。
...
- [実行方法](#実行方法)
- [アーキテクチャ](#アーキテクチャ)
- [使用した LLM API と選定理由](#使用したllm-apiと選定理由)
- [ツール一覧](#ツール一覧)
...
このエージェントは、与えられた評価をただ文章化するのではなく、**自ら過去の評価履歴を取得・比較した上で**フィードバックを生成する。「LLM を 1 回呼ぶ API」ではなく、判断とツール呼び出しを反復する**エージェントループ**として実装している。
...
2. デモルートからログイン（認証不要）
3. スタッフ詳細ページを開く
4. 「AI フィードバックを生成」ボタンを押下
5. 数秒後、フィードバックが表示される
6. 「AI の処理過程を表示」を開くと、呼び出されたツール・生成コスト・生成時間が確認できる

**本番環境**: https://growth-finder-psi.vercel.app
...
- `status = 'completed'` の評価のみを対象とし、draft を読み込まない
...
数値の集計・傾向判定はコード側（ルールベース）で行い、LLM には計算をさせない。理由はコスト抑制に加え、LLM に数値計算を任せると不正確になりうるため。LLM は集計結果の**解釈と言葉選び**に専念する。
...
LLM エージェントの実装は今回が初めてであり、一つひとつの設計判断を検証しながら進めた。以下はその思考の過程である。
...
さらに、方針 A により最新評価は props で受け取っているため、`data[0]`（クエリ上の最新）と
