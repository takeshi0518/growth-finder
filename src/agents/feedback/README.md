...
- [AI フィードバック生成エージェント](#ai-フィードバック生成エージェント)
  ...
  - [セットアップと実行方法](#セットアップと実行方法)
    ...
    - [ローカルで動かす](#ローカルで動かす)
  - [必要な環境変数](#必要な環境変数)
  - [人間の承認と権限の切り分け](#人間の承認と権限の切り分け)
    - [AI は入口にも出口にも立たない](#ai-は入口にも出口にも立たない)
    - [構造として権限を制限している](#構造として権限を制限している)
    - [なぜ人間が必要なのか](#なぜ人間が必要なのか)
  - [アーキテクチャ](#アーキテクチャ)
    - [プロンプト設計](#プロンプト設計)
...
評価を下すのは店長であり、AI ではない。QHC のスコアも総括コメントも、すべて人間が入力する。
生成されたフィードバックは店長が確認したうえで面談に使う。AI が直接スタッフに伝えることはない。
AI が担うのは、店長の評価を承認の言葉に変換する中間の工程だけである。

### 構造として権限を制限している

この役割分担は運用ルールではなく、実装の構造で担保している。

- **ツールは read-only** — AI はデータを書き換えられない。評価を修正する権限がない
- **`input_schema` は空** — AI は対象スタッフを選べない。staffId は Server Action が保持する
- **DB 層でも制限している** — ツールは anon key 経由で Supabase にアクセスし、
  RLS により店長自身の店舗のスタッフ評価しか取得できない。
  仮にアプリケーション層の制御をすり抜けても、他店舗のデータには到達しない
- **`MAX_ITERATIONS = 5`** — ツール呼び出しの反復に上限を設けている

### なぜ人間が必要なのか

AI はツールであり、人間を支援するものだと考えている。領域によっては、AI の方が速く、質の高い成果を出す。
「Todo アプリを作って」と指示すれば数分で動くものが出てくる。人間には到底できない。
それは優劣ではなく、得意不得意である。

一方で、表情や空気感といった言語化しにくいものを頼りに人を動かすことは、人間にしかできない。
そして本プロダクトの目的である「承認」は、まさにその領域にある。

人は誰かに認められると嬉しい。これは根源的な欲求だと思っている。
そして仕事は、どんな仕事であっても社会貢献の一部である。その活動の中で認められることは、
SNS などで得られる承認とは比較にならないほど深く刺さる。
だからこそ、職場で認められた人はその職場に留まる。承認が定着に効くのは、この構造による。

**AI にフィードバックをもらっても、承認欲求は満たされない。**

承認とは、自分を見ていた誰かに認められることである。毎日一緒に働き、努力を見てきた店長が
「成長したね」と言うから意味がある。AI は見ていない。どれだけ精度が上がっても、AI の言葉は承認にならない。

つまり、人間を入口と出口に置いているのは、安全性や精度のためではない。
そうしなければ、このプロダクトが目的を達成できないからである。

技術的には、生成したフィードバックをそのままスタッフに送信したり、
スタッフ専用ページで閲覧させたりすることは容易である。実装コストも低く、
「自動化」という観点ではその方が徹底している。店長が確認する手間すら省ける。

しかし、それをすればこのプロダクトは意味を失う。
スタッフがページを開いて AI の生成した文章を読んでも、それは承認ではなく通知である。
店長が面談の場で、相手の表情を見ながら伝えるからこそ、承認として機能する。

AI は店長が承認するための材料を整えるが、承認そのものは店長にしかできない。
だから本エージェントは、フィードバックを生成して店長に見せるところで止まる。
...
プロンプトは `src/agents/feedback/prompt.ts` の `buildPrompt` で構築する。構成は以下の 4 層。

**役割の定義** — 「評価を下すのは店長であり、あなたではない」「評価そのものを判断したり、
点数を付け直したりしてはいけない」と明示する。ツールなしで動かした際にハルシネーションが
発生した経験から、AI が評価を捏造しないよう役割を明確に縛っている。

**今回の評価データ** — `formatCurrentEvaluation` の結果を埋め込む。総括コメントが未記入の場合は
「（記入なし）」として渡し、null がそのまま文字列化されないようにしている。

**ツールの使い方** — どのツールをどういう場面で使うかを示す。ツール定義の `description` と
併せて、LLM がツールを呼ぶ動機づけを行う。

**フィードバックの方針と出力形式** — 承認のトーン、字数、話し言葉であること。
加えて禁止事項として、前置き（「データを確認しました」等）、Markdown 記法、
文字数の報告などを明示的に禁じている。

実装当初は「前置きは不要」といった否定形の指示だけでは効かず、LLM が
「データを確認しました。それでは作成します」といった作業報告を出力し続けた。
「スタッフへの呼びかけから書き始めてください」と**書き出しを指定する肯定形の指示**を加え、
禁止事項を独立したセクションとして具体例つきで列挙したことで解消した。
...
以下は今回実装していない。実務投入するなら対応が必要になる。

- **タイムアウト** — 明示的な設定はしておらず、Anthropic SDK のデフォルト挙動に依存している。
  LLM の応答が長時間返らない場合の制御ができていない。
- **自動リトライ** — 一時的な失敗に対する再試行を実装していない。生成に失敗した場合、
  店長が UI から再度ボタンを押す形になる。途中状態からの再開もできず、最初からやり直しになる。
...
今回最も苦労したのは、AI エージェントという未知の技術を既存プロジェクトへ実装。

AI エージェントの実装経験がなかったため、まずは概念やアーキテクチャを理解するところから始めた。
